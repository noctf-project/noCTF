...
const DEFAULT_CONFIG = (r: FastifyRequest): RateLimitBucket | undefined => {
  const key = r.routeOptions.url
...
export const RateLimitHook = async (
  ...
  if (
    await policyService.evaluate(request.user?.id ?? 0, ["bypass.rate_limit"])
  ) {
    return;
  }
  ...
  if (typeof config === "function") {
    ...
    if (Array.isArray(derived)) {
      buckets = derived.filter((x) => x !== undefined);
    } else if (derived) {
