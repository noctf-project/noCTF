<script lang="ts">
    ...
    const allFalse = Object.fromEntries(Array.from(categories).map((k) => [k, false]))
    ...
    const getCategoryCounts = (category: string) => {
        const chals = allChallenges.filter((c) => c.categories.find((f) => f == category))
        return { solved: chals.filter((c) => c.isSolved).length, total: chals.length }
    }

    $effect(() => {
        const filtered = anyFilter ? allChallenges : allChallenges.filter((chal) => {
            const f = new Set(Object.keys(categoryFilters).filter((k) => categoryFilters[k]))
...
{#snippet categoryBtn(category: string, solved: number, total: number)}
    {@const btnClass = 'btn btn-ghost font-bold border border-base-500 shadow-solid w-9/12 max-w-64 flex flex-row pl-2 gap-3 justify-start'}
    {@const selected = (category == 'All' && anyFilter) || categoryFilters[category]}
    <button
    ...
    >
        ...
        <div class={selected ? 'text-gray-200' : 'text-gray-500'}>{solved}/{total}</div>
    </button>
{/snippet}
