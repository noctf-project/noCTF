<script lang="ts">
    ...
    const challenges: ChallengeEntry[] = [
        ...
        { id: 8, title: 'XSS-blog this chall title long', points: 400, categories: ['web'] },
    ...
    const scoreboard = [
        { team: 'qqq', rank: 1, score: 3850, solves: [0, 1, 2, 3, 4, 6, 7, 9, 10, 12, 13] },
        { team: 'long team name long team name', rank: 2, score: 3600, solves: [0, 1, 3, 4, 5, 6, 7, 9, 12] },
        { team: 'l33t h4x0rs', rank: 3, score: 3400, solves: [0, 1, 2, 3, 6, 7, 9, 12, 13] },
        { team: 'Binary Bandits', rank: 4, score: 3200, solves: [0, 3, 4, 6, 7, 8, 9, 12] },
        { team: 'Segmentation Fault', rank: 5, score: 2900, solves: [0, 1, 3, 6, 7, 9, 12] },
        { team: 'Stack Smashers', rank: 6, score: 2700, solves: [0, 3, 4, 6, 9, 12, 13] },
        { team: 'Buffer Overflow', rank: 7, score: 2500, solves: [0, 1, 3, 6, 7, 12] },
...
<div class="flex flex-row gap-0 w-full ml-[26.5rem]">
    ...
    <thead class="h-4">
        <tr>
            ...
            {#each challenges as challenge}
                <th class="border border-base-300 bg-base-200 w-12 text-center text-sm">
                    {challenge.points}
                </th>
            {/each}
        </tr>
        ...
        {#each scoreboard as entry}
            <tr>
                ...
                {#each challenges as challenge}
                    ...
                    <td class={'border border-base-300 p-0 ' + (solved ? 'bg-primary/20' : '')}>
                        <div class="w-full h-full flex items-center justify-center">
                            {#if solved}
                                <Icon icon="material-symbols:flag" class="text-primary text-3xl" />
                            {/if}
                        </div>
                    </td>
                {/each}
