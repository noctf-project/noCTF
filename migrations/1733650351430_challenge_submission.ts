...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await schema.createTable("challenge")
    ...
    .execute();
  await schema.createIndex("challenge_idx_tags")
    .on("challenge")
    .using('gin')
    .column("tags")
    .execute();
  await schema.createIndex("challenge_idx_hidden_visible_at")
    .on("challenge")
  ...
  await schema.createTable("submission")
    ...
    .execute();
  await schema.createIndex("submission_idx_pending")
    .on("submission")
    .columns(["pending"])
    .execute();

    await schema.createIndex("submission_idx_created_at_challenge_id")
    .on("submission")
    .expression(sql`created_at DESC, challenge_id`)
    .execute();
  
  await schema.createTable("solve")
    .addColumn("id", "integer", (col) =>
    ...
    .addColumn("hidden", "boolean", (col) => col.notNull().defaultTo(false))
    .addUniqueConstraint("uidx_challenge_id_team_id", ["challenge_id", "team_id"])
    .execute();
}
