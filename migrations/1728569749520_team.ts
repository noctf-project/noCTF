...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await schema.createTable('team_member')
    .addColumn('user_id', 'integer', (col) => col.primaryKey().references('core.user.id'))
    .addColumn('team_id', 'integer', (col) => col.notNull().references('core.team.id'))
    .addColumn('role', sql`core.team_member_role`, (col) => col.notNull().defaultTo('member'))
    .addColumn('created_at', 'timestamp', (col) =>
      col.defaultTo(sql`now()`).notNull(),
    )
    .execute();
