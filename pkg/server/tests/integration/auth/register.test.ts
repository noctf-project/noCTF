...
test('/register', async (t) => {
  ...
  t.equal(newUser.statusCode, 200);
  t.match(data.token, /.+/);

  t.test('a taken email', async (tt) => {
    ...
    tt.equal(response.statusCode, 409);
    tt.equal(response.json().detail?.key, 'email');
  });

  t.test('a taken username', async (tt) => {
    const response = await app.inject({
      method: 'post',
      path: '/api/auth/register',
      payload: JSON.stringify({
    ...
    tt.equal(response.statusCode, 409);
    tt.equal(response.json().detail?.key, 'name');
  });
});
