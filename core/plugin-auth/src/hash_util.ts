...
const SCryptValidator = async (password: string, parts: string[]) => {
  ...
  for (let [k, v] of optionsStr.split(',')
    .map((opt) => opt.split("="))) {
    let numVal = parseInt(v);
    if (isNaN(numVal) || !Number.isInteger(numVal) || numVal <= 0) {
      throw new Error(`invalid value for parameter ${k} passed to scrypt validator`);
    }
...
const VALIDATORS: Record<string, (password: string, parts: string[]) => Promise<boolean>> = {
...
export const Generate = async (password: string) => {
  ...
  const key = await scryptPromise(password, salt, SCRYPT_KEYLEN, SCRYPT_OPTIONS);
