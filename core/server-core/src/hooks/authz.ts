...
export const AuthzHook = async (request: FastifyRequest) => {
  ...
  if (!request.user) {
    if (
      !(await evaluateCached((await roleService.getStaticRoleIds()).public))
    ) {
  ...
  try {
    await Promise.any(
      (await roleService.getUserRoleIds(request.user.id)).map((role) =>
        evaluateCached(role).then((r) =>
          r ? Promise.resolve() : Promise.reject(),
        ),
      ),
    );
  } catch {
